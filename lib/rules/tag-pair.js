exports.init = function (THERAHint) {
	THERAHint.addRule({
        ...
        if (mapEmptyTags[tagName]) {
          reporter.warn(`${tagName} is a self-close tag, no need end tag: [` + event.raw + ' ]', event.line, event.col, self, event.raw);
          return;
				}
				...
				if (pos >= 0) {
					...
					if(arrTags.length > 0){
						...
						reporter.error('Tag must be paired, missing: [ '+ arrTags.join('') + ' ], start tag match failed [ ' + lastEvent.raw + ' ] on line ' + lastEvent.line + '.', lastEvent.line, lastEvent.raw.col, self, event.raw);
					}
					stack.length=pos;
				} else {
			...
			parser.addListener('end', function(event){
				...
				if(arrTags.length > 0){
					var lastEvent = stack[stack.length-1];
					reporter.error('Tag must be paired, missing: [ '+ arrTags.join('') + ' ], open tag match failed [ ' + lastEvent.raw + ' ] on line ' + lastEvent.line + '.', lastEvent.line, lastEvent.raw.col, self, '');
				}
			});
		}
	});
