						...
						try{
							...
							if(status === false){
								jsVerify.errors.forEach(function (error) {
									var line = error.line;
									var logger = reporter.error;
									if ("W" == error.code[0]) {
										logger = reporter.warn;
									} else if ("E" == error.code[0]) {
										logger = reporter.error;
									}
									logger = logger.bind(reporter);
									logger(error.reason, styleLine + line, (line === 1 ? styleCol : 0) + error.character, self, error.evidence);
								});
							}
						}
						catch(e){}
