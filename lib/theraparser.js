var THERAParser = (function(undefined){

	var THERAParser = function(){
		var self = this;
		self._init.apply(self,arguments);
	};

	THERAParser.prototype = {
		_init: function(){
			var self = this;
			self._listeners = {};
		...
		parse: function(weex) {
			...
			while ((match = regTag.exec(weex))) {
				matchIndex = match.index;
				if (matchIndex > lastIndex) { //保存前面的文本或者CDATA
					text = weex.substring(lastIndex, matchIndex);
					if (tagCDATA) {
						arrCDATA.push(text);
					} else { //文本
						var jsxTag = /(\{\{)(?:[^\}]*\}?[^\}]*)(\}\})/g;
						...
						while ((m = jsxTag.exec(text))) {
							saveBlock('text', text.slice(i, m.index), i + lastIndex);
							saveBlock('jsx', m[0], m.index + lastIndex);
							i = m.index + m[0].length;
						}
						
						saveBlock('text', text.slice(i), i + lastIndex);
					}
				}
			...
			if(weex.length > lastIndex){
				//结尾文本
				text = weex.substring(lastIndex, weex.length);
			...
			self.fire('end', {
				pos: lastIndex,
				line: line,
				col: weex.length - lastLineIndex + 1
			});
...
if (typeof exports === 'object' && exports){
	exports.THERAParser = THERAParser;
}
